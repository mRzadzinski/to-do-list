...
const dateTimeHandler = (() => {
			...
			div.addEventListener('click', () => {
				timePickers.forEach((picker) => {
					if (div.contains(picker)) {
						...
						dateDisplayDiv = currentPicker.previousElementSibling;
					}
				});
			});
			div.addEventListener('change', () =>
				convertDate(currentPicker.value, dateDisplayDiv)
			);
	...
	return { addDateListeners, toggleDate, convertDate };
})();
...
const expandElementHandler = (() => {
	...
	listNameHeader.onclick = () =>
		expandElement(chooseDropdown('dropdown-task-list'));
	...
	function addTaskMenuListeners() {
		...
		taskMenuButtons.forEach((button) =>
			button.addEventListener('click', (event) => {
				...
				let taskPositionY = lastUsedTaskMenuBtn.getBoundingClientRect().y;
				// Prevent menu from overflowing screen
				if (
					document.body.scrollHeight - taskPositionY <
					taskMenu.scrollHeight
				) {
					taskMenu.style.top =
						document.body.scrollHeight - taskMenu.scrollHeight + 'px';
				} else {
					taskMenu.style.top =
						lastUsedTaskMenuBtn.getBoundingClientRect().y + 'px';
				}
	...
	document.onclick = (event) => {
		dropdowns.forEach((dropdown) => {
			if (dropdown.classList.contains('active')) {
				if (
					!listNameHeader.contains(event.target) &&
					dropdown.classList.contains('dropdown-task-list')
				) {
					...
					lastUsedTaskMenuBtn &&
					!lastUsedTaskMenuBtn.contains(event.target) &&
...
const dragAndDropHandler = (() => {
	...
	function handleDrop(e) {
		...
		if (
			dragElement !== this &&
			!lastTask.classList.contains('dragover-bottom-border')
		) {
...
function renderTaskLists() {
	...
	while (taskListBreakLine.previousSibling)
		taskListBreakLine.previousSibling.remove();
...
const completedList = document.querySelector('.completed-list');
const completedTaskTemplate = document.querySelector(
...
function renderTasks() {
	...
	for (const task in currentList.tasks) {
		...
		if (
			task &&
			typeof currentList.tasks[task] === 'object' &&
			...
			newTask.children[0].children[1].children[0].innerHTML =
				currentList.tasks[task].name;
			newTask.children[0].children[1].children[0].nextElementSibling.value =
				currentList.tasks[task].name;

			newTask.children[1].children[0].innerHTML =
				currentList.tasks[task].details;
			newTask.children[1].children[0].nextElementSibling.value =
				currentList.tasks[task].details;
...
function selectNewTask() {
	...
	allTasks.forEach((task) => {
		...
		if (
			...
			taskContent === '' &&
			taskDetails === '' &&
			taskDateDiv.classList.contains('hidden')
		) {
			task.classList.add('task-clicked');
