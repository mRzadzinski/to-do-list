...
function loadLocalStorage() {
	if (!localStorage.getItem('taskLists')) {
		...
		let loadedCurrentListName = JSON.parse(
			localStorage.getItem('currentListName')
		);
...
function createDefaultList() {
	taskLists.push(ListFactory('List'));
}
...
[addTaskIcon, addTaskText].forEach((element) =>
...
function increaseTasksPosition() {
	for (let task in currentList.tasks) {
		if (
			...
			!currentList.tasks[task].completed
		) {
...
function deleteTask(taskID) {
	...
	for (let task in currentList.tasks) {
		if (
			currentList.tasks[task].position >
			currentList.tasks[taskToDeleteName].position
		) {
			currentList.tasks[task].position--;
		}
	}
		...
		if (currentList.tasks[task].id == taskID) {
			if (currentList.tasks[task].completed === false) {
				...
				for (let tk in currentList.tasks) {
					if (
						currentList.tasks[tk].position > currentList.tasks[task].position
					) {
	...
	if (newPosition === 'before') {
		sortedTasksArray.forEach((task) => {
			if (taskToMovePosition > dropTargetPosition) {
				if (
					task[1].position >= dropTargetPosition &&
					task[1].position < taskToMovePosition
				) {
					task[1].position++;
